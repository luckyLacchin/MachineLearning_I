...
def gen_lin_sep_dataset(n_samples, guarantee_separable=False, center_dist=3.0):
    """
    ...
    """
    y_multi = 2
    D, c_idx = datasets.make_blobs(n_samples=[n_samples // 2, n_samples // 2], centers=[[center_dist/2, 0], [-center_dist/2, 0]],
                                   n_features=2)

    # Remove samples that destroy separability if required.
    if guarantee_separable:
        sep_idx = [i for i in range(len(D)) if D[i,0] < 0 and c_idx[i] == 1 or D[i,0] > 0 and c_idx[i] == 0] #if statement is the condition to be satisfied in order to add i into the list
        D = D[sep_idx]
        c_idx = c_idx[sep_idx]
