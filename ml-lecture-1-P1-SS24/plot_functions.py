...
def get_angle_plot(line1, line2, offset=0.1, color=None, origin=(0, 0), len_x_axis = 1, len_y_axis = 1):
    ...
    #return Arc(origin, len_x_axis*offset, len_y_axis*offset, angle=angle, theta1=theta1, theta2=theta2)
    return Arc(origin, len_x_axis*offset, len_y_axis*offset, 0, theta1, theta2, color=color, label = str(angle)+u"\u00b0") #it has been written by the prof. with errors
...
def init_2D_linear_separation_plot(C, NotC, w, R):
    ...
    # Draw vector w*
    # w_arrow, = plt.plot([0, 0], [w[0], w[1]], 'k', label=f"w=({w[0]}, {w[1]})")
